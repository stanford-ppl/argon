...
abstract class Transformer { self =>
  ...
  def apply[T](xs: List[Exp[T]]): List[Exp[T]] = xs.map { x =>
    this.apply(x)
  }
  def apply[T](xs: Seq[Exp[T]]): Seq[Exp[T]] = xs.map { x =>
    this.apply(x)
  }
  def apply[T](x: Option[Exp[T]]): Option[Exp[T]] = x.map { z =>
    this.apply(z)
  }

  def tx(xs: List[Exp[_]]): List[Exp[_]] = xs.map { x =>
    f(x)
  }
  def tx(xs: Set[Exp[_]]): Set[Exp[_]] = xs.map { x =>
    f(x)
  }
  def tx(xs: Seq[Exp[_]]): Seq[Exp[_]] = xs.map { x =>
    f(x)
  }

  def txSyms(xs: Set[Sym[_]]): Set[Sym[_]] =
    syms(xs.map { x =>
      f(x)
    }).toSet
  ...
  final protected def transferMetadataIfNew(lhs: Seq[Exp[_]])(
      tx: => Seq[Exp[_]]): (Seq[Exp[_]], Seq[Boolean]) = {
    val id   = IR.curEdgeId
    val lhs2 = tx
  ...
  final protected def transferMetadataIfNew[T](lhs: Exp[T])(
      tx: => Exp[T]): (Exp[T], Boolean) = {
    val id   = IR.curEdgeId
    val lhs2 = tx
    (lhs, lhs2) match {
      case (sym: Exp[_], sym2: Sym[_]) if sym2.id >= id || sym == sym2 =>
  ...
  final def mirror(lhs: Seq[Sym[_]], rhs: Def): Seq[Exp[_]] = {
    ...
    lhs.foreach { s =>
      ...
      metadata.get(s).foreach { m =>
        log(c" - ${m._1}: ${m._2}")
      }
    }

    val (lhs2, _) = transferMetadataIfNew(lhs) {
      rhs.mirrorNode(lhs, self.asInstanceOf[Tx])
    }

    log(c"Result: ${str(lhs2)}")
    lhs2.foreach { s2 =>
      if (lhs2.length > 1) log(c"$s2")
      metadata.get(s2).foreach { m =>
        log(c" - ${m._1}: ${m._2}")
      }
    }
  ...
  final def mirror(
      props: Map[Class[_], Metadata[_]]): Map[Class[_], Metadata[_]] = {
    ...
    props.collect {
      case (key, meta) if !meta.ignoreOnTransform => key -> mirror(meta)
    }.toMap
  }
  final def mirror[M <: Metadata[_]](m: M): M =
    m.mirror(self.asInstanceOf[Tx]).asInstanceOf[M]
