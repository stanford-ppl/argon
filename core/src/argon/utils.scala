...
object utils {
  ...
  def deleteExts(path: String, ext: String) = {
    val files: Array[String] = Option(new File(path).list)
      .map(_.filter(_.endsWith(ext)))
  ...
  object single {
    ...
    def collectSeq[T](blk: PartialFunction[Any, T])(x: Any): Seq[T] = {
      ...
      singleVisit(x) { x =>
        if (blk.isDefinedAt(x)) out += blk(x)
      }
      out.result
    }
    ...
    def collectSeqs[T](blk: PartialFunction[Any, Iterable[T]])(
        x: Any): Seq[T] = {
      val out = new mutable.ListBuffer[T]
      singleVisit(x) { x =>
        if (blk.isDefinedAt(x)) out ++= blk(x)
    ...
    def collectSets[T](blk: PartialFunction[Any, Iterable[T]])(
        x: Any): Set[T] = {
      val out = new mutable.SetBuilder[T, Set[T]](Set.empty[T])
      singleVisit(x) { x =>
        if (blk.isDefinedAt(x)) out ++= blk(x)
  ...
  object recursive {
    private def recurseVisit[T](x: Any)(
        func: PartialFunction[Any, Unit]): Unit = x match {
      case e if func.isDefinedAt(e) => func(e)
      case ss: Iterator[_] =>
        while (ss.hasNext) { recurseVisit(ss.next)(func) }
