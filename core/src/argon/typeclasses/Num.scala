  ...
  implicit def num2num[T: Meta: Num, R: Meta: Num] = new Cast[T, R] {
    def apply(x: T)(implicit ctx: SrcCtx): R = typ[R] match {
      case tp: FixPtType[s, i, f] =>
        num[T].toFixPt(x)(tp.mS, tp.mI, tp.mF, ctx).asInstanceOf[R]
      case tp: FltPtType[g, e] =>
        num[T].toFltPt(x)(tp.mG, tp.mE, ctx).asInstanceOf[R]
...
trait NumExp
    ...
    with BitsExp
    with OrderExp
    with LowPriorityNumImplicits {
  this: BoolExp with FixPtExp with FltPtExp with CastExp =>

  trait Num[T] extends Bits[T] with Arith[T] with Order[T] {
    def toFixPt[S: BOOL, I: INT, F: INT](x: T)(
  ...
  implicit def num2fltpt[T: Num, G: INT, E: INT] = new Cast[T, FltPt[G, E]] {
    def apply(x: T)(implicit ctx: SrcCtx): FltPt[G, E] =
      num[T].toFltPt[G, E](x)
  }
  implicit def num2fixpt[T: Num, S: BOOL, I: INT, F: INT] =
    new Cast[T, FixPt[S, I, F]] {
      def apply(x: T)(implicit ctx: SrcCtx): FixPt[S, I, F] =
        num[T].toFixPt[S, I, F](x)
    }
