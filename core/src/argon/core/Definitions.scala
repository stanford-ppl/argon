...
trait Definitions extends Blocks { self: Staging =>
  ...
  abstract class Def extends Node with Product {
    ...
    final def allInputs: Seq[Dyn[_]] = nodeInputs(this.id).map { id =>
      symFromSymId(id)
    }
    final def expInputs: Seq[Exp[_]] =
      recursive.collectSeqs(__exps)(productIterator)
    ...
    def blocks: Seq[Block[_]] =
      recursive.collectSeq { case b: Block[_] => b }(productIterator)
    ...
    def tunnels: Seq[(Dyn[_], Seq[Dyn[_]])] =
      blocks
        .flatMap { blk =>
    ...
    def aliases: Seq[Dyn[_]] =
      recursive.collectSeq {
        case s: Dyn[_] if outputTypes.contains(s.tp) => s
      }(productIterator)
    ...
    def mutate(f: Tx): Unit =
      throw new Exception("Cannot mutate immutable node")
  ...
  abstract class Op3[A: Type, B: Type, R: Type] extends Op2[A, R] {
    def mB = typ[B]
  }
  abstract class Op4[A: Type, B: Type, C: Type, R: Type] extends Op3[A, B, R] {
    def mC = typ[C]
  }
  abstract class Op5[A: Type, B: Type, C: Type, D: Type, R: Type]
      extends Op4[A, B, C, R] { def mD = typ[D] }
  ...
  def getDef(s: Exp[_]): Option[Def] = s match {
    case s: Sym[_] => Some(defOf(s)); case _ => None
  }
  ...
  final def exps(a: Any*): Seq[Exp[_]] =
    if (__exps.isDefinedAt(a)) __exps(a) else Nil
  final def dyns(a: Any*): Seq[Dyn[_]] =
    if (__dyns.isDefinedAt(a)) __dyns(a) else Nil
  final def syms(a: Any*): Seq[Sym[_]] = dyns(a).collect {
    case s: Sym[_] => s
  }

  private def symsFreq(a: Any*): Seq[(Dyn[_], UseFreq)] =
    recursive.collectSeqs {
  ...
  final def hot(e: Any*) = symsFreq(e: _*).map {
    case (s, f) => (s, combine(f, Freq.Hot))
  }
  final def cold(e: Any*) = symsFreq(e: _*).map {
    case (s, f) => (s, combine(f, Freq.Cold))
  }

  final def aliasSyms(a: Any): Set[Dyn[_]] =
    recursive.collectSets {
      case s: Dyn[_] => Set(s)
      case d: Def    => d.aliases
    }(a)
  final def containSyms(a: Any): Set[Dyn[_]] =
    recursive.collectSets { case d: Def => d.contains }(a)
  final def extractSyms(a: Any): Set[Dyn[_]] =
    recursive.collectSets { case d: Def => d.extracts }(a)
  final def copySyms(a: Any): Set[Dyn[_]] =
    recursive.collectSets { case d: Def => d.copies }(a)
}
