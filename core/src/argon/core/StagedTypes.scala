...
trait StagedTypes extends EmbeddedControls { this: Staging =>
  ...
  @implicitNotFound(msg =
    "Type ${T} is not a staged type. Try adding an explicit lift() call?")
  ...
  private def unstagedWarning(op: String)(implicit ctx: SrcCtx): Unit = {
    warn(ctx,
         s"Unstaged method $op was used here on a staged type during staging.")
  ...
  private def unstagedWarningNoCtx(op: String)(ctx: SrcCtx): Unit = {
    ...
    val name = "the value"
    warn(ctx,
         s"Unstaged method $op was used on $name defined here during staging.")
    warn("Add @virtualize annotation to an enclosing scope to prevent this.")
    warn(ctx)
  }
  ...
  @util
  def __equals[A, T <: MetaAny[T]](x: A, y: T)(
      implicit lift: Lift[A, T]): Bool = lift(x) === y
  @util
  def __equals[A, T <: MetaAny[T]](x: T, y: A)(
      implicit lift: Lift[A, T]): Bool                    = x === lift(y)
  @util def __unequals[T <: MetaAny[T]](x: T, y: T): Bool = x =!= y
  @util
  def __unequals[A, T <: MetaAny[T]](x: A, y: T)(
      implicit lift: Lift[A, T]): Bool = lift(x) =!= y
  @util
  def __unequals[A, T <: MetaAny[T]](x: T, y: A)(
      implicit lift: Lift[A, T]): Bool = x =!= lift(y)
  ...
  def wrap[T: Meta](xs: List[Exp[T]]): List[T] = xs.map { t =>
    meta[T].wrapped(t)
  }
  def wrap[T: Meta](xs: Seq[Exp[T]]): Seq[T] = xs.map { t =>
    meta[T].wrapped(t)
  }

  def unwrap[T: Meta](x: T): Exp[T] = meta[T].unwrapped(x)
  def unwrap[T: Meta](xs: Seq[T]): Seq[Exp[T]] = xs.map { t =>
    meta[T].unwrapped(t)
  }
  def unwrap[T: Meta](xs: List[T]): List[Exp[T]] = xs.map { t =>
    meta[T].unwrapped(t)
  }
  ...
  def infix_==[A <: MetaAny[A], B <: MetaAny[B]](x1: A, x2: B): Bool =
    macro equalImpl[Bool]
  def infix_==[A, B <: MetaAny[B]](x1: B, x2: A)(
      implicit l: Lift[A, B]): Bool = x1 === lift(x2)
  def infix_==[A, B <: MetaAny[B]](x1: A, x2: B)(
      implicit l: Lift[A, B]): Bool = lift(x1) === x2

  def infix_!=[A <: MetaAny[A], B <: MetaAny[B]](x1: A, x2: B): Bool =
    macro unequalImpl[Bool]
  def infix_!=[A, B <: MetaAny[B]](x1: B, x2: A)(
      implicit l: Lift[A, B]): Bool = x1 =!= lift(x2)
  def infix_!=[A, B <: MetaAny[B]](x1: A, x2: B)(
      implicit l: Lift[A, B]): Bool = lift(x1) =!= x2
  ...
  @implicitNotFound(
    msg =
      "Cannot find way to lift type ${A} to type ${B}. Try adding an explicit cast using .to[${B}].")
...
private object StagedTypes {
  def equalImpl[T](c: whitebox.Context)(x1: c.Expr[Any],
                                        x2: c.Expr[Any]): c.Expr[T] = {
    import c.universe._
    c.Expr(q"$x1 === $x2")
  }

  def equalLiftRightImpl[T](c: whitebox.Context)(
      x1: c.Expr[Any],
      x2: c.Expr[Any])(l: c.Tree): c.Expr[T] = {
    import c.universe._
    c.Expr(q"$x1 === lift($x2)")
  }

  def equalLiftLeftImpl[T](c: whitebox.Context)(
      x1: c.Expr[Any],
      x2: c.Expr[Any])(l: c.Tree): c.Expr[T] = {
    import c.universe._
    c.Expr(q"lift($x1) === $x2")
  ...
  def unequalImpl[T](c: whitebox.Context)(x1: c.Expr[Any],
                                          x2: c.Expr[Any]): c.Expr[T] = {
    import c.universe._
    c.Expr(q"__unequals($x1, $x2)")
  }

  def unequalLiftRightImpl[T](c: whitebox.Context)(
      x1: c.Expr[Any],
      x2: c.Expr[Any])(l: c.Tree): c.Expr[T] = {
    import c.universe._
    c.Expr(q"$x1 =!= lift($x2)")
  ...
  def unequalLiftLeftImpl[T](c: whitebox.Context)(
      x1: c.Expr[Any],
      x2: c.Expr[Any])(l: c.Tree): c.Expr[T] = {
    import c.universe._
    c.Expr(q"lift($x1) =!= $x2")
