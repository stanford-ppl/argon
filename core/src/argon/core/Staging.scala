...
trait Staging extends Scheduling {
  ...
  def stageDefPure(d: Def)(ctx: SrcCtx): Seq[Sym[_]] =
    stageDefEffectful(d, Pure)(ctx)
  def stageDefCold(d: Def)(ctx: SrcCtx): Seq[Sym[_]] =
    stageDefEffectful(d, Cold)(ctx)
  def stageDefWrite(ss: Exp[_]*)(d: Def)(ctx: SrcCtx): Seq[Sym[_]] =
    stageDefEffectful(d, Write(ss: _*))(ctx)
  def stageDefSimple(d: Def)(ctx: SrcCtx): Seq[Sym[_]] =
    stageDefEffectful(d, Simple)(ctx)
  def stageDefGlobal(d: Def)(ctx: SrcCtx): Seq[Sym[_]] =
    stageDefEffectful(d, Global)(ctx)
  def stageDefMutable(d: Def)(ctx: SrcCtx): Seq[Sym[_]] =
    stageDefEffectful(d, Mutable)(ctx)

  def stage[T: Type](op: Op[T])(ctx: SrcCtx): Sym[T] =
    single[T](stageDef(op)(ctx))
  def stagePure[T: Type](op: Op[T])(ctx: SrcCtx): Sym[T] =
    single[T](stageDefPure(op)(ctx))
  def stageCold[T: Type](op: Op[T])(ctx: SrcCtx): Sym[T] =
    single[T](stageDefCold(op)(ctx))
  def stageWrite[T: Type](ss: Exp[_]*)(op: Op[T])(ctx: SrcCtx): Sym[T] =
    single[T](stageDefWrite(ss: _*)(op)(ctx))
  def stageSimple[T: Type](op: Op[T])(ctx: SrcCtx): Sym[T] =
    single[T](stageDefSimple(op)(ctx))
  def stageGlobal[T: Type](op: Op[T])(ctx: SrcCtx): Sym[T] =
    single[T](stageDefGlobal(op)(ctx))
  def stageMutable[T: Type](op: Op[T])(ctx: SrcCtx): Sym[T] =
    single[T](stageDefMutable(op)(ctx))
  def stageEffectful[T: Type](op: Op[T], u: Effects)(ctx: SrcCtx): Sym[T] =
    single[T](stageDefEffectful(op, u)(ctx))
  ...
  private def registerDef(d: Def, extraDeps: Seq[Sym[_]])(
      ctx: SrcCtx): Seq[Sym[Any]] = {
    val bounds  = d.binds
    val tunnels = d.tunnels
    val dfreqs = d.freqs
      .groupBy(_._1)
      .mapValues(_.map(_._2).fold(Freq.Normal) { combine })
    val freqs = d.inputs.map { in =>
      dfreqs.getOrElse(in, Freq.Normal)
    } ++ extraDeps.distinct.map { d =>
      Freq.Normal
    }

    val inputs = d.inputs
    val outputs = d.outputTypes.map { tp =>
      new Sym(tp)
    }
  ...
  def stageDefEffectful(d: Def, u: Effects)(ctx: SrcCtx): Seq[Sym[_]] = {
    ...
    if (effects == Pure) registerDefWithCSE(d)(ctx)
      ...
      def stageEffects(): Seq[Sym[_]] = {
        ...
        if (aliases.nonEmpty)
          new IllegalMutableSharingError(ss.head, aliases)(ctx)
        if (immutables.nonEmpty)
          new IllegalMutationError(ss.head, immutables)(ctx)
  ...
  private def single[T: Type](xx: Seq[Sym[_]]): Sym[T] =
    xx.head.asInstanceOf[Sym[T]]
