...
trait Symbols extends StagedTypes with Metadata with Graph { self: Staging =>
  ...
  case class SrcCtxs(pos: List[SrcCtx]) extends Metadata[SrcCtxs] {
    def mirror(f: Tx) = this
  }
  object ctxsOf {
    def apply(x: Exp[_]): List[SrcCtx] =
      metadata[SrcCtxs](x).map(_.pos).getOrElse(Nil)
  ...
  implicit class SrcCtxOps(x: Exp[_]) {
    ...
    def addCtx(ctx: SrcCtx) {
      if (ctx != EmptyContext || ctxsOf(x).isEmpty)
        ctxsOf(x) = ctxsOf(x) :+ ctx
    }
    def setCtx(ctx: SrcCtx) {
      if (ctx != EmptyContext || ctxsOf(x).isEmpty) ctxsOf(x) = List(ctx)
    }
  }

  case class CtxName(name: String) extends Metadata[CtxName] {
    def mirror(f: Tx) = this
  }
  ...
  /** Any staged symbol **/
  sealed abstract class Exp[+T] private[core] (staged: Type[T])
      extends EdgeLike {
    def tp: Type[T @uncheckedVariance] = staged
  }

  /** A staged symbol which represents a non-constant value **/
  sealed abstract class Dyn[+T] private[core] (staged: Type[T])
      extends Exp[T](staged)
      with Edge {
    ...
    def dependents: Seq[Exp[_]] =
      dependentsOf(this.id).flatMap(nodeOutputs).map(symFromSymId)
  }
  ...
  /** A staged constant **/
  class Const[+T] private[core] (x: Any)(staged: Type[T])
      extends Exp[T](staged) {
  ...
  class Param[+T] private[core] (x: Any)(staged: Type[T])
      extends Const[T](x)(staged) {
  ...
  object Param {
    def unapply(s: Exp[_]): Option[Any] = s match {
      case param: Param[_] =>
        Some(param.c) // TODO: Should this only return if isFinal is false?
      case _ => None
    }
  }
