...
trait Reporting {
  ...
  def plural(x: Int, sing: String, plur: String): String =
    if (x == 1) sing else plur
  ...
  final def log(x: => Any): Unit =
    if (Config.verbosity >= 2) logstream.println(x)
  final def dbg(x: => Any): Unit =
    if (Config.verbosity >= 1) logstream.println(x)
  final def msg(x: => Any): Unit = {
    logstream.println(x)
    if (Config.verbosity >= 2) System.out.println(x)
  }

  final def report(x: => Any): Unit =
    if (Config.verbosity >= 0) System.out.println(x)
  ...
  final def warn(ctx: SourceContext,
                 x: => Any,
                 noWarn: Boolean = false): Unit = {
    warn(ctx.toString() + ": " + x)
    if (!noWarn) _warns += 1
  }
  final def error(ctx: SourceContext,
                  x: => Any,
                  noError: Boolean = false): Unit = {
    error(ctx.fileName + ":" + ctx.line + ": " + x)
    if (!noError) _errors += 1
  }

  final def warn(ctx: SourceContext, showCaret: Boolean): Unit =
    if (ctx.lineContent.isDefined) {
      warn(ctx.lineContent.get)
      if (showCaret) warn(" " * (ctx.column - 1) + "^") else warn("")
    }
  final def error(ctx: SourceContext, showCaret: Boolean): Unit =
    if (ctx.lineContent.isDefined) {
  ...
  def readable(x: Any): String = x match {
    ...
    case p: Iterable[_] =>
      if (p.isEmpty) "Nil" else p.map(readable).mkString("Seq(", ",", ")")
    case p: Product =>
      if (p.productIterator.isEmpty) c"${p.productPrefix}"
      else
        c"""${p.productPrefix}(${p.productIterator
          .map(readable)
          .mkString(", ")})"""
    case _ =>
