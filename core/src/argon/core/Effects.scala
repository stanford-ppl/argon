...
trait Effects extends Symbols { this: Staging =>
  ...
  final def checkContext(): Unit =
    if (context == null) throw new UninitializedEffectContextException()
  ...
  object depsOf {
    ...
    def update(x: Exp[_], deps: Seq[Exp[_]]) =
      metadata.add(x, Dependencies(deps))
  }
  ...
  case class Effects(
    ...
    def mirror(f: Tx) =
      Effects(cold, simple, global, mutable, f.txSyms(reads), f.txSyms(writes))
    ...
    def isPure =
      !cold && !simple && !global && !mutable && reads.isEmpty && writes.isEmpty
    def isMutable = mutable
    def isIdempotent =
      !cold && !simple && !global && !mutable && writes.isEmpty
    def mayWrite(ss: Set[Sym[_]]) = global || ss.exists { s =>
      writes contains s
    }
    def mayRead(ss: Set[Sym[_]]) = global || ss.exists { s =>
      reads contains s
    }
  }
  val Pure                   = Effects()
  ...
  final def isMutable(s: Exp[_]): Boolean =
    metadata[Effects](s).exists(_.mutable)
  ...
  /**
    ...
    */
  final def effectDependencies(effects: Effects): Seq[Sym[_]] =
    if (effects.global) context
    else {
      ...
      def isAAWHazard(u: Effects) = {
        if (unwrittenAccesses.nonEmpty) {
          val (written, unwritten) =
            unwrittenAccesses.partition(u.writes.contains)
          unwrittenAccesses = unwritten
          written.nonEmpty
        } else false
      }

      val hazards = context.filter {
        case e @ Effectful(u, _) =>
          isWARHazard(u) || isAAWHazard(u) || (accesses contains e)
      }
      val simpleDep = if (effects.simple) context.find {
        case Effectful(u, _) => u.simple
      } else None // simple
      ...
      if (e == Pure) "Pure"
        ...
        "(" +
          ((if (e.cold) List(c"cold=${e.cold}") else Nil) ++
            ...
            (if (e.reads.nonEmpty)
               List(c"""reads={${e.reads.map(readable).mkString(",")}}""")
             else Nil) ++
            (if (e.writes.nonEmpty)
               List(c"""writes={${e.writes.map(readable).mkString(",")}}""")
             else Nil)).mkString(", ") + ")"
      }
