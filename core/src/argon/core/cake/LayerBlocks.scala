...
trait LayerBlocks { self: ArgonCore =>
  ...
  @stateful def stageLambda1[A,R](a: Exp[A])(block: => Exp[R], temp: Freq = Freq.Normal): Lambda1[A,R] = {
    val (result, effects, effectful) = stageScope(block, temp)
    Lambda1(a, result, effects, effectful, temp)
  }
  @stateful def stageLambda2[A,B,R](a: Exp[A], b: Exp[B])(block: => Exp[R], temp: Freq = Freq.Normal): Lambda2[A,B,R] = {
    val (result, effects, effectful) = stageScope(block, temp)
    Lambda2(a, b, result, effects, effectful, temp)
  }
  @stateful def stageLambda3[A,B,C,R](a: Exp[A], b: Exp[B], c: Exp[C])(block: => Exp[R], temp: Freq = Freq.Normal): Lambda3[A,B,C,R] = {
    val (result, effects, effectful) = stageScope(block, temp)
    Lambda3(a, b, c, result, effects, effectful, temp)
  }
  @stateful def stageLambda4[A,B,C,D,R](a: Exp[A], b: Exp[B], c: Exp[C], d: Exp[D])(block: => Exp[R], temp: Freq = Freq.Normal): Lambda4[A,B,C,D,R] = {
    val (result, effects, effectful) = stageScope(block, temp)
