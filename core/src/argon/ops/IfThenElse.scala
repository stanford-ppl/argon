...
trait IfThenElseApi extends IfThenElseExp with BoolApi { this: TextApi =>
}
...
trait IfThenElseExp
    ...
    with BoolExp
    with VoidExp
    with OverloadHack { this: TextExp =>

  /** Virtualized Methods **/
  @util
  def __ifThenElse[A, B, T <: MetaAny[T]](
      cond: Bool,
      thenp: => A,
      elsep: => B)(implicit liftA: Lift[A, T], liftB: Lift[B, T]): T = {
  ...
  @util
  def __ifThenElse[A, T <: MetaAny[T]](cond: Bool, thenp: => A, elsep: => T)(
      implicit lift: Cast[A, T],
      x1: Overload1): T = {
  ...
  @util
  def __ifThenElse[A, T <: MetaAny[T]](cond: Bool, thenp: => T, elsep: => A)(
      implicit lift: Cast[A, T],
      x2: Overload2): T = {
    implicit val staged: Meta[T] = lift.staged
  ...
  @util
  def __ifThenElse[T <: MetaAny[T]: Meta](cond: Bool,
                                          thenp: => T,
                                          elsep: => T): T = {
    val unwrapThen
      : () => Exp[T] = () => thenp.s // directly calling unwrap(thenp) forces thenp to be evaluated here
    val unwrapElse
      : () => Exp[T] = () => elsep.s // wrapping it as a Function0 allows it to be delayed
    wrap(ifThenElse(cond.s, unwrapThen(), unwrapElse()))
  }
  ...
  /** IR Nodes **/
  case class IfThenElse[T: Type](cond: Exp[Bool],
                                 thenp: Block[T],
                                 elsep: Block[T])
      extends Op[T] {
  ...
  /** Constructors **/
  def ifThenElse[T: Type](cond: Exp[Bool], thenp: => Exp[T], elsep: => Exp[T])(
      implicit ctx: SrcCtx): Exp[T] = cond match {
    case Const(true) if context != null =>
      thenp // Inlining is not valid if there is no outer context
