...
trait HashMapApi extends HashMapExp with ArrayApi with StructApi {
  ...
  implicit class ArrayGroupByOps[A](array: MetaArray[A]) {
    @api
    def groupByReduce[K: Meta, V: Meta](key: A => K)(value: A => V)(
        reduce: (V, V) => V)(implicit ctx: SrcCtx,
                             mA: Meta[A]): MetaHashMap[K, V] = {
      val i       = fresh[Index]
      ...
      val out =
        stageDefEffectful(BuildHashMap(array.s, aBlk, kBlk, vBlk, rBlk, rV, i),
                          effects.star)(ctx)
...
trait HashMapExp extends Staging with ArrayExp with StructExp {
  ...
  /** Infix methods **/
  case class HashIndex[K: Meta](s: Exp[HashIndex[K]])
      extends MetaAny[HashIndex[K]] {
    @api def =!=(x: HashIndex[K]): Bool =
      ??? // TODO! but never seen by user currently
    @api def ===(x: HashIndex[K]): Bool =
      ??? // TODO! but never seen by user currently
    @api def toText: Text = textify(this)
  }

  case class MetaHashMap[K: Meta, V: Meta](s: Exp[MetaHashMap[K, V]])
      extends MetaStruct[MetaHashMap[K, V]] {
    ...
    private def getIndex(key: K)(implicit ctx: SrcCtx): Index =
      wrap(hash_index_apply(this.index.s, key.s))
  ...
  case class HashMapType[K, V](mK: Meta[K], mV: Meta[V])
      extends StructType[MetaHashMap[K, V]] {
    ...
    override def isPrimitive                        = true
    override def fields =
      Seq("keys"   -> ArrayType(mK),
          "values" -> ArrayType(mV),
          "index"  -> HashIndexType(mK),
          "size"   -> IntType)
  }
  implicit def stagedMap[K: Meta, V: Meta]: StructType[MetaHashMap[K, V]] =
    HashMapType(meta[K], meta[V])

  /** IR Nodes **/
  // Gets an integer key entry from a hash index, -1 if it is not present
  case class HashIndexApply[K: Type](index: Exp[HashIndex[K]], key: Exp[K])
      extends Op[Index] {
  ...
  case class HashMapNew[K: Type, V: Type](
    ...
    def elems =
      Seq("keys" -> keys, "values" -> values, "index" -> index, "size" -> size)
  ...
  case class BuildHashMap[A: Meta, K: Meta, V: Meta](
    ...
    def outputTypes =
      List(ArrayType(meta[K]), ArrayType(meta[V]), HashIndexType(meta[K]))

    override def inputs =
      dyns(in) ++ dyns(apply) ++ dyns(keyFunc) ++ dyns(valFunc) ++ dyns(reduce)
    override def freqs =
      normal(in) ++ hot(apply) ++ hot(keyFunc) ++ hot(valFunc) ++ hot(reduce)
  ...
  /** Constructors **/
  protected def hash_index_apply[K: Type](
      index: Exp[HashIndex[K]],
      key: Exp[K])(implicit ctx: SrcCtx): Exp[Index] = {
  ...
  private def build_hashmap[A: Meta, K: Meta, V: Meta](
    ...
    : (Exp[MetaArray[K]], Exp[MetaArray[V]], Exp[HashIndex[K]]) = {
    ...
    val effects = aBlk.summary andAlso kBlk.summary andAlso vBlk.summary andAlso rBlk.summary
    val out =
      stageDefEffectful(BuildHashMap(in, aBlk, kBlk, vBlk, rBlk, rV, i),
                        effects.star)(ctx)
