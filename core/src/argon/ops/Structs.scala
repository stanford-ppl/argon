...
trait StructApi extends StructExp with VoidApi { self: TextApi =>
}
...
trait StructExp extends Staging with VoidExp with TextExp {
  ...
  abstract class MetaStruct[T: StructType] extends MetaAny[T] { self =>
    ...
    protected def field[R: Meta](name: String)(implicit ctx: SrcCtx): R =
      wrap(field_apply[T, R](self.s, name))
    protected def fieldToText[T](name: String, tp: Meta[T])(
        implicit ctx: SrcCtx) = tp.ev(field(name)(mtyp(tp), ctx)).toText

    def fields(implicit ctx: SrcCtx): Seq[(String, MetaAny[_])] = {
      tp.fields.map {
        case (name, fieldTyp) =>
          name -> fieldTyp.ev(field(name)(mtyp(fieldTyp), ctx))
      }
    }
    ...
    @api def toText = {
      val fields = tp.fields.map {
        case (name, fieldTyp) => fieldToText(name, fieldTyp)
      }
      lift[String, Text](tp.prefix + "(") + fields.reduceLeft { (a, b) =>
        a + "," + b
      } + ")"
    }
  }
  ...
  abstract class StructType[T](override implicit val ev: T <:< MetaStruct[T])
      extends Meta[T] {
  ...
  def struct[T: StructType](fields: (String, Exp[_])*)(
      implicit ctx: SrcCtx): T = wrap(struct_new[T](fields))
  def field[T: StructType, R: Meta](struct: T, name: String)(
      implicit ctx: SrcCtx): R = wrap(field_apply[T, R](struct.s, name))
  ...
  case class SimpleStruct[S: StructType](elems: Seq[(String, Exp[_])])
      extends StructAlloc[S] {
    def mirror(f: Tx) =
      struct_new[S](elems.map { case (idx, sym) => idx -> f(sym) })
  }

  case class FieldApply[S: StructType, T: Type](struct: Exp[S], field: String)
      extends Op2[S, T] {
    def mirror(f: Tx) = field_apply[S, T](f(struct), field)

    override def extracts = dyns(struct)
  }
  case class FieldUpdate[S: StructType, T: Type](struct: Exp[S],
                                                 field: String,
                                                 value: Exp[T])
      extends Op3[S, T, Void] {
  ...
  /** Constructors **/
  def struct_new[S: StructType](elems: Seq[(String, Exp[_])])(
      implicit ctx: SrcCtx): Exp[S] = {
    stage(SimpleStruct(elems))(ctx)
  }

  // TODO: Should struct unwrapping be disabled for mutable structs?
  def field_apply[S: StructType, T: Type](struct: Exp[S], index: String)(
      implicit ctx: SrcCtx): Exp[T] = struct match {
    case Op(s: StructAlloc[_]) if Config.unwrapStructs =>
      unwrapStruct[S, T](struct, index) match {
        case Some(x) => x
        case None    => stage(FieldApply[S, T](struct, index))(ctx)
      }
    case _ => stage(FieldApply[S, T](struct, index))(ctx)
  }
  def field_update[S: StructType, T: Type](
      struct: Exp[S],
      index: String,
      value: Exp[T])(implicit ctx: SrcCtx): Exp[Void] = {
  ...
  def unwrapStruct[S: StructType, T: Type](struct: Exp[S],
                                           index: String): Option[Exp[T]] =
    struct match {
      case Op(Struct(elems)) =>
        elems.get(index) match {
          case Some(x) if x.tp <:< typ[T] =>
            Some(x.asInstanceOf[Exp[T]]) // TODO: Should this be Staged asInstanceOf?
          case None =>
