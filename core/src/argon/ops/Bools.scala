...
trait BoolExp extends Staging with BitsExp with CastExp { this: TextExp =>
  ...
  implicit object BoolBits extends Bits[Bool] {
    ...
    override def random(max: Option[Bool])(implicit ctx: SrcCtx): Bool =
      Bool(bool_random(max.map(_.s)))
  ...
  /** Constant lifting **/
  implicit def boolean2bool(x: Boolean)(implicit ctx: SrcCtx): Bool =
    Bool(bool(x))
  def bool(x: Boolean)(implicit ctx: SrcCtx): Exp[Bool] = constant[Bool](x)

  /** Casts **/
  implicit object Text2Bool extends Cast[Text, Bool] {
    override def apply(x: Text)(implicit ctx: SrcCtx): Bool =
      Bool(text_to_bool(x.s))
  }

  /** IR Nodes **/
  case class Not(a: Exp[Bool]) extends Op[Bool] {
    def mirror(f: Tx) = bool_not(f(a))
  }
  case class And(a: Exp[Bool], b: Exp[Bool]) extends Op[Bool] {
    def mirror(f: Tx) = bool_and(f(a), f(b))
  }
  case class Or(a: Exp[Bool], b: Exp[Bool]) extends Op[Bool] {
    def mirror(f: Tx) = bool_or(f(a), f(b))
  }
  case class XOr(a: Exp[Bool], b: Exp[Bool]) extends Op[Bool] {
    def mirror(f: Tx) = bool_xor(f(a), f(b))
  }
  case class XNor(a: Exp[Bool], b: Exp[Bool]) extends Op[Bool] {
    def mirror(f: Tx) = bool_xnor(f(a), f(b))
  }
  case class RandomBool(max: Option[Exp[Bool]]) extends Op[Bool] {
    def mirror(f: Tx) = bool_random(f(max))
  }
  ...
  def bool_and(x: Exp[Bool], y: Exp[Bool])(implicit ctx: SrcCtx): Exp[Bool] =
    (x, y) match {
      case (Const(a: Boolean), Const(b: Boolean)) =>
        bool(a && b) // Constant propagation
      case (Const(false), _) =>
        bool(false) // Short circuit evaluation: false && _ => false
      case (_, Const(false)) =>
        bool(false) // Short circuit evaluation: _ && false => false
  ...
  def bool_or(x: Exp[Bool], y: Exp[Bool])(implicit ctx: SrcCtx): Exp[Bool] =
    (x, y) match {
      case (Const(a: Boolean), Const(b: Boolean)) =>
        bool(a || b) // Constant propagation
      case (Const(true), _) =>
        bool(true) // Short circuit evaluation: true || _ => true
      case (_, Const(true)) =>
        bool(true) // Short circuit evaluation: _ || true => true
  ...
  def bool_xor(x: Exp[Bool], y: Exp[Bool])(implicit ctx: SrcCtx): Exp[Bool] =
    (x, y) match {
      case (Const(a: Boolean), Const(b: Boolean)) =>
        bool(a != b) // Constant propagation
      case (Const(false), b) => b // Boolean simplification: false != b => b
      case (Const(true), b) =>
        stage(Not(b))(ctx) // Boolean simplification: true != b => !b
      case (a, Const(false)) => a // Boolean simplification: a != false => a
      case (a, Const(true)) =>
        stage(Not(a))(ctx) // Boolean simplification: a != true => !a
      case _ => stage(XOr(x, y))(ctx) // Default constructor
    }
  def bool_xnor(x: Exp[Bool], y: Exp[Bool])(implicit ctx: SrcCtx): Exp[Bool] =
    (x, y) match {
      case (Const(a: Boolean), Const(b: Boolean)) =>
        bool(a == b) // Constant propagation
      case (Const(false), b) =>
        stage(Not(b))(ctx) // Boolean simplification: false == b => !b
      case (Const(true), b) => b // Boolean simplification: true == b => b
      case (a, Const(false)) =>
        stage(Not(a))(ctx) // Boolean simplification: a == false => !a
      case (a, Const(true)) => a // Boolean simplification: a == true => a
      case _                => stage(XNor(x, y))(ctx) // Default constructor
    }
  def bool_random(max: Option[Exp[Bool]])(implicit ctx: SrcCtx): Exp[Bool] =
    max match {
