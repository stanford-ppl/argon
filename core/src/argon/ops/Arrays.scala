...
trait ArrayExp
  ...
  /** Infix methods **/
  case class MetaArray[T: Meta](s: Exp[MetaArray[T]])
      extends MetaAny[MetaArray[T]] {
    @api def apply(i: Int32): T = wrap(array_apply(s, i.s))
    @api def length: Int32      = wrap(array_length(s))

    @api def ===(that: MetaArray[T]): Bool = {
      val eqs = array_infix_zip(this, that, { (x: T, y: T) =>
        x === y
      })
      array_infix_reduce(eqs, { (a: Bool, b: Bool) =>
        a && b
      })
    }
    @api def =!=(that: MetaArray[T]): Bool = {
      val neqs = array_infix_zip(this, that, { (x: T, y: T) =>
        x =!= y
      })
      array_infix_reduce(neqs, { (a: Bool, b: Bool) =>
        a || b
      })
    }
  ...
  private[argon] def array_infix_update[T: Meta](
      array: MetaArray[T],
      i: Index,
      data: T)(implicit ctx: SrcCtx): Void = {
    Void(array_update(array.s, i.s, data.s))
  }
  private[argon] def array_from_function[T: Meta](
      size: Index,
      func: Index => T)(implicit ctx: SrcCtx): MetaArray[T] = {
    val i    = fresh[Index]
    val fBlk = () => func(wrap(i)).s
    MetaArray(array_mapindices(size.s, fBlk(), i))
  }
  private[argon] def array_infix_foreach[T: Meta](
      array: MetaArray[T],
      func: T => Void)(implicit ctx: SrcCtx): Void = {
    val i       = fresh[Index]
    ...
    Void(
      stageEffectful(ArrayForeach(array.s, aBlk, fBlk, i), effects.star)(ctx))
  }
  private[argon] def array_infix_map[T: Meta, R: Meta](
      array: MetaArray[T],
      func: T => R)(implicit ctx: SrcCtx): MetaArray[R] = {
    val i       = fresh[Index]
    val aBlk    = stageLambda(array.s) { array.apply(wrap(i)).s }
    val fBlk    = stageLambda(aBlk.result) { func(wrap(aBlk.result)).s }
    val effects = aBlk.summary andAlso fBlk.summary
    val out =
      stageEffectful(ArrayMap(array.s, aBlk, fBlk, i), effects.star)(ctx)
    MetaArray(out)
  }
  private[argon] def array_infix_zip[T: Meta, S: Meta, R: Meta](
      a: MetaArray[T],
      b: MetaArray[S],
      func: (T, S) => R)(implicit ctx: SrcCtx): MetaArray[R] = {
    val i    = fresh[Index]
    val aBlk = stageLambda(a.s) { a.apply(wrap(i)).s }
    val bBlk = stageLambda(b.s) { b.apply(wrap(i)).s }
    val fBlk = stageLambda(aBlk.result, bBlk.result) {
      func(wrap(aBlk.result), wrap(bBlk.result)).s
    }
    val effects = aBlk.summary andAlso bBlk.summary andAlso fBlk.summary
    val out = stageEffectful(ArrayZip(a.s, b.s, aBlk, bBlk, fBlk, i),
                             effects.star)(ctx)
    MetaArray(out)
  }
  private[argon] def array_infix_reduce[T: Meta](
      array: MetaArray[T],
      reduce: (T, T) => T)(implicit ctx: SrcCtx): T = {
    val i       = fresh[Index]
    val rV      = (fresh[T], fresh[T])
    val aBlk    = stageLambda(array.s) { array.apply(wrap(i)).s }
    val rBlk    = stageBlock { reduce(wrap(rV._1), wrap(rV._2)).s }
    val effects = aBlk.summary andAlso rBlk.summary
    val out = stageEffectful(ArrayReduce(array.s, aBlk, rBlk, i, rV),
                             effects.star)(ctx)
    wrap(out)
  }
  private[argon] def array_infix_filter[T: Meta](
      array: MetaArray[T],
      filter: T => Bool)(implicit ctx: SrcCtx): MetaArray[T] = {
    val i       = fresh[Index]
    val aBlk    = stageLambda(array.s) { array.apply(wrap(i)).s }
    val cBlk    = stageLambda(aBlk.result) { filter(wrap(aBlk.result)).s }
    val effects = aBlk.summary andAlso cBlk.summary
    val out =
      stageEffectful(ArrayFilter(array.s, aBlk, cBlk, i), effects.star)(ctx)
    MetaArray(out)
  }
  private[argon] def array_infix_flatMap[T: Meta, R: Meta](
      array: MetaArray[T],
      func: T => MetaArray[R])(implicit ctx: SrcCtx): MetaArray[R] = {
    val i       = fresh[Index]
    val aBlk    = stageLambda(array.s) { array.apply(wrap(i)).s }
    val fBlk    = stageLambda(aBlk.result) { func(wrap(aBlk.result)).s }
    val effects = aBlk.summary andAlso fBlk.summary
    val out =
      stageEffectful(ArrayFlatMap(array.s, aBlk, fBlk, i), effects.star)(ctx)
    MetaArray(out)
  }

  private[argon] def input_arguments()(ctx: SrcCtx): MetaArray[Text] =
    wrap(stage(InputArguments())(ctx))

  /** Type classes **/
  // --- Staged
  case class ArrayType[T](child: Meta[T]) extends Meta[MetaArray[T]] {
    override def wrapped(s: Exp[MetaArray[T]]): MetaArray[T] =
      MetaArray(s)(child)
  ...
  case class ArrayApply[T: Type](array: Exp[MetaArray[T]], i: Exp[Int32])
      extends Op[T] {
  ...
  case class ArrayUpdate[T: Type](array: Exp[MetaArray[T]],
                                  i: Exp[Int32],
                                  e: Exp[T])
      extends Op[Void] {
  ...
  case class MapIndices[T: Type](size: Exp[Index],
                                 func: Block[T],
                                 i: Bound[Index])
      extends Op[MetaArray[T]] {
  ...
  case class ArrayZip[A: Type, B: Type, C: Type](
    ...
    def mirror(f: Tx) =
      array_zip(f(arrayA), f(arrayB), f(applyA), f(applyB), f(func), i)
    override def inputs =
      dyns(arrayA) ++ dyns(arrayB) ++ dyns(applyA) ++ dyns(applyB) ++ dyns(
        func)
    override def freqs =
      normal(arrayA) ++ normal(arrayB) ++ hot(applyA) ++ hot(applyB) ++ hot(
        func)
    override def binds = i +: super.binds
  ...
  /** Constructors **/
  def array_new[T: Type](size: Exp[Int32])(
      implicit ctx: SrcCtx): Sym[MetaArray[T]] = {
    stageMutable(ArrayNew[T](size))(ctx)
  }
  def array_apply[T: Type](array: Exp[MetaArray[T]], i: Exp[Int32])(
      implicit ctx: SrcCtx): Sym[T] = {
    stage(ArrayApply(array, i))(ctx)
  }
  def array_update[T: Type](array: Exp[MetaArray[T]],
                            i: Exp[Int32],
                            e: Exp[T])(implicit ctx: SrcCtx): Sym[Void] = {
    stageWrite(array)(ArrayUpdate(array, i, e))(ctx)
  }
  def array_length[T: Type](array: Exp[MetaArray[T]])(
      implicit ctx: SrcCtx): Sym[Int32] = {
