...
class UninitializedEffectContextException()(implicit state: State) extends
  CompilerException(1, "Attempted to stage effectful node outside effect context"){ def console() = () }

case class RedefinedSymbolException(s: Sym[_], d1: Def, d2: Def)(implicit state: State) extends
  CompilerException(2, c"Symbol $s was redefined during staging"){def console() = {
...
class CodegenException(codegen: String, lhs: Sym[_], rhs: Def)(implicit state: State) extends
  CompilerException(4, c"Don't know how to generate $codegen code for $lhs = $rhs"){def console() = {
    error(c"[$codegen] Don't know how to generate code for $lhs = $rhs")
  }}

class EffectsOrderException(implicit state: State) extends CompilerException(5, c"Violated ordering of effects"){ def console() = () }

class FlexEvaluationException(s: Sym[_])(implicit state: State) extends
  CompilerException(6, c"Unable to evaluate $s"){ def console() = error(c"Unable to flex evaluate ${str(s)}") }

class UndefinedMirrorException(x: Any)(implicit state: State) extends
  CompilerException(7, c"Undefined mirroring for $x"){ def console() = () }

class TraversalFailedToConvergeException(msg: String)(implicit state: State) extends
  CompilerException(9, msg){ def console() = error(msg) }

class TraversalFailedToCompleteException(msg: String)(implicit state: State) extends
  CompilerException(10, msg){ def console() = error(msg) }
...
class IllegalSubstException(name: String, s: Exp[_], prev: Exp[_], attempted: Exp[_])(implicit state: State) extends
  CompilerException(12, c"Transformer $name encountered illegal substitution"){ def console() = {
...
class IllegalMirrorExpException(name: String, s: Exp[_])(implicit state: State) extends
  CompilerException(13, c"Transformer $name could not directly mirror single symbol $s - has fat definition"){ def console() = {
...
class PretransformException(name: String, s: Exp[_], s2: Exp[_])(implicit state: State) extends
  CompilerException(14, c"Transformer $name encounted symbol $s with existing rule $s -> $s2."){ def console() = {
...
class UndefinedAccessPatternException(x: Exp[_])(implicit state: State) extends
  CompilerException(15, c"Symbol $x has no defined access pattern"){ def console() = {
    error(c"Symbol $x has no defined access pattern")
  }}

class NoBitWidthException(tp: Type[_])(implicit state: State) extends
  CompilerException(16, c"Type $tp has no method for computing bit width"){ def console() = {
...
class UnsupportedBankingType(name: String, x: Exp[_])(implicit state: State) extends
  CompilerException(17, c"Memory $x has no rule for $name banking"){ def console() = {
...
class TupleSizeUnsupported(element: String, x: Exp[_])(implicit state: State) extends
  CompilerException(18, c"FieldApply $x has no rule for applying to $element th element"){ def console() = {
...
class NDArrayException(array: Exp[_], name: String)(implicit state: State) extends
  CompilerException(21, c"Array $array ($name) has no good codegen for CPP"){ def console() = {
...
class AccumWithoutReduceFunctionException(reg: Exp[_], lhs: Exp[_])(implicit state: State) extends
  CompilerException(19, c"Register $reg claims to be an accumulator but has no reduction function assigned"){ def console() = {
...
CompilerException(1025, c"""Cannot create new wire for $lhs"""){ def console() = {
