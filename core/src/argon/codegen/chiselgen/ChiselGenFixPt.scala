...
trait ChiselGenFixPt extends ChiselCodegen {
  ...
  override protected def quoteConst(c: Const[_]): String = (c.tp, c) match {
    ...
    case (FixPtType(s, d, f), Const(cc: BigDecimal)) =>
      if (needsFPType(c.tp)) { s"Utils.FixedPoint($s,$d,$f,$cc)" } else {
        if (cc >= 0) cc.toInt.toString + ".U(32.W)"
        else cc.toInt.toString + ".S(32.W).asUInt"
      }
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case FixMul(x, y) =>
      alphaconv_register(src"$lhs"); emit(src"val $lhs = $x * $y")
    ...
    case FixConvert(x) =>
      lhs.tp match {
        ...
        case FixPtType(s, d, f) =>
          emit(src"val $lhs = $x // should be fixpt ${lhs.tp}")
      }
    case FixPtToFltPt(x) =>
      lhs.tp match {
