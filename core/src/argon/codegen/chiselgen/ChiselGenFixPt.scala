...
trait ChiselGenFixPt extends ChiselCodegen {
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case FixMul(x,y) => alphaconv_register(src"$lhs"); emitGlobalWireMap(src"$lhs", src"Wire(${newWireFix(lhs.tp)})");emit(src"${lhs}.r := ($x *-* $y).r")
    case FixDiv(x,y) => emitGlobalWireMap(src"$lhs", src"Wire(${newWireFix(lhs.tp)})");emit(src"${lhs}.r := ($x /-/ $y).r")
    ...
    case FixMod(x,y) => emitGlobalWireMap(src"$lhs",src"Wire(${newWireFix(lhs.tp)})");emit(src"$lhs := $x %-% $y")
