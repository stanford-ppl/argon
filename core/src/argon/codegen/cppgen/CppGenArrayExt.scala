...
trait CppGenArrayExt extends CppGenArray {
  ...
  protected def emitUpdate(lhs: Exp[_],
                           value: Exp[_],
                           i: String,
                           tp: Type[_]): Unit = {
    if (isArrayType(tp)) {
      ...
      open(
        src"for (int ${value}_copier = 0; ${value}_copier < ${getSize(value)}; ${value}_copier++) {")
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    case ArrayUpdate(array, i, data) =>
      emitUpdate(array, data, src"$i", data.tp)
    case MapIndices(size, func, i) =>
      emitNewArray(lhs, lhs.tp, src"$size")
      open(src"for (int $i = 0; $i < $size; ${i}++) {")
      emitBlock(func)
      if (isArrayType(func.result.tp)) {
        if (getNestingLevel(func.result.tp) > 1) {
          Console.println(s"ERROR: Need to fix more than 2D arrays")
        }
        emit(src"(*$lhs)[$i].resize(${getSize(func.result)});")
        open(
          src"for (int ${func.result}_copier = 0; ${func.result}_copier < ${getSize(
            func.result)}; ${func.result}_copier++) {")
        emit(
          src"(*$lhs)[$i][${func.result}_copier] = (*${func.result})[${func.result}_copier];")
        close("}")
      } else {
    ...
    case ArrayReduce(array, apply, reduce, i, rV) =>
      if (isArrayType(lhs.tp)) {
        emit(
          src"""${lhs.tp}* $lhs = new ${lhs.tp}(${getSize(array, "[0]")});""")
      } else {
        emit(src"${lhs.tp} $lhs;")
      }
      open(
        src"if (${getSize(array)} > 0) { // Hack to handle reductions on things of length 0")
      ...
      emit(
        src"""${rV._2.tp}${if (isArrayType(rV._2.tp)) "*" else ""} ${rV._2} = $lhs;""")
      emitBlock(reduce)
    ...
    case ArrayFlatMap(array, apply, func, i) =>
      ...
      emit(
        "// TODO: flatMap node assumes the func block contains only applies (.flatten)")

      // Initialize lhs array
      (0 until nesting).map { level =>
        val grabbers = (0 until level)
          .map { m =>
            "[0]"
          }
          .mkString("")
      ...
      emitNewArray(lhs, lhs.tp, src"""${(0 until nesting)
        .map { m =>
          src"size_${lhs}_$m"
        }
        .mkString("*")}""")
      ...
      (0 until nesting).foreach { level =>
        ...
        open(
          src"for (int ${i}_$level = 0; ${i}_${level} < size_${lhs}_$level; ${i}_${level}++) { ")
      }

      // Pluck off elements of the $array
      val applyString = (0 until nesting)
        .map { level =>
          src"""[${i}_${level}]"""
        }
        .mkString("")
      emit(
        src"${getPrimitiveType(lhs.tp)} ${func.result} = (*${array})${applyString};")

      // Update the lhs
      val flatIndex = (0 until nesting)
        .map { level =>
          src"""${(level + 1 until nesting)
            .map { k =>
              src"size_${lhs}_$k"
            }
            .mkString("*")} ${if (level + 1 < nesting) "*" else ""}${i}_${level}"""
        }
        .mkString(" + ")
