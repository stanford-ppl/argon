...
trait CppGenHashMap extends CppGenArrayExt {
  ...
  override protected def emitFat(lhs: Seq[Sym[_]], rhs: Def) = rhs match {
    case e @ BuildHashMap(in, apply, keyFunc, valFunc, reduce, rV, i) =>
      emit(
        src"vector<${e.mK}>* ${lhs(0)} = new vector<${e.mK}>((*${in}).size()); // Keys")
      emit(
        src"vector<${e.mV}>* ${lhs(1)} = new vector<${e.mV}>((*${in}).size()); // Values")
      emit(
        src"vector<${e.mK}>* ${lhs(2)} = ${lhs(0)}; // TODO: Probably totally wrong.  lhs2 appears to be the scala hashmap, but it seems like we only use it to lookup index of a key")
      ...
      emit(s"long idx = -1;")
      emit(
        src"for (int i = 0; i < ${lhs(2)}_size; i++) {if ((*${lhs(0)})[i] == ${keyFunc.result}) {contained = true; idx = i;}}")
      open(src"if (contained) {")
      emit(src"${rV._1.tp}* ${rV._1} = ${valFunc.result};")

      emit(
        src"${rV._2.tp}* ${rV._2} = new ${rV._2.tp}((*${lhs(1)})[idx].size());")
      open(
        src"for (int ${rV._2}_copier = 0; ${rV._2}_copier < ${getSize(rV._2)}; ${rV._2}_copier++) {")
      emit(
        src"(*${rV._2})[${rV._2}_copier] = (*${lhs(1)})[idx][${rV._2}_copier];")
      close("}")
