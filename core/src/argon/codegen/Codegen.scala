...
trait Codegen extends Traversal {
  ...
  protected def emit(x: String, forceful: Boolean = false): Unit = {
    if (emitEn | forceful) {
      ...
      if (Config.emitDevel == 2) {
        Console.println(
          s"[ ${lang}gen-NOTE ] Emission of ${x} does not belong in this backend")
      }
    }
  }
  protected def open(x: String): Unit = {
    if (emitEn) {
      stream.println(tabbed + x);
      if (streamTab contains streamName) streamTab(streamName) += 1
    } else {
      if (Config.emitDevel == 2) {
        Console.println(
          s"[ ${lang}gen-NOTE ] Emission of ${x} does not belong in this backend")
  ...
  protected def closeopen(x: String): Unit = { // Good for "} else {" lines
    if (emitEn) {
      if (streamTab contains streamName) streamTab(streamName) -= 1;
      stream.println(tabbed + x); streamTab(streamName) += 1
    } else {
      if (Config.emitDevel == 2) {
        Console.println(
          s"[ ${lang}gen-NOTE ] Emission of ${x} does not belong in this backend")
  ...
  protected def spatialNeedsFPType(tp: Type[_]): Boolean =
    false // TODO: Spatial specific!
  ...
  final protected def withStream[A](out: PrintWriter)(body: => A): A = {
    ...
    try { body } finally {
      stream.flush(); stream = save; streamName = saveName
    }
  }

  final protected def newStream(name: String,
                                exten: String = ext): PrintWriter = {
  ...
  final protected def newTemplate(name: String,
                                  exten: String = ext): PrintWriter = {
    // TODO: Assert streamMap does not contain this guy already
    val fullname = name + "." + exten
    streamTab += (fullname -> 0)
  ...
  final protected def getStream(name: String, exten: String = ext)
    : PrintWriter = { // Use stream if it exists, otherwise maek it exist
    // TODO: Assert streamMap does not contain this guy already
    val fullname = name + "." + exten
  ...
  protected def quoteConst(c: Const[_]): String = {
    if (Config.emitDevel > 0) {
      if (emitEn) { // Want to emit but can't
        ...
        Console.println(
          s"[ ${lang}gen-NOTE ] Quoting of $c does not belong in this backend")
      }
  ...
  protected def quoteOrRemap(arg: Any): String = arg match {
    ...
    case _ =>
      throw new RuntimeException(
        s"Could not quote or remap $arg (${arg.getClass})")
  }
  ...
  protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = {
    if (emitEn) {
      if (Config.emitDevel == 0) {
        ...
        Console.println(
          s"[ ${lang}gen-ERROR ] no backend for $lhs = $rhs in $lang")
      }
    } else {
      if (Config.emitDevel == 2)
        Console.println(
          s"[ ${lang}gen-NOTE ] Emission of ${lhs} = $rhs does not belong in this backend")
    }
  }

  protected def emitFat(lhs: Seq[Sym[_]], rhs: Def): Unit =
    throw new GenerationFailedException(rhs)
