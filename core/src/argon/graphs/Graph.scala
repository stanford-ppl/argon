...
trait Graph extends Exceptions {
  ...
  var graphLog: PrintStream =
    if (VERBOSE_SCHEDULING)
      createLog(Config.logDir + "/sched/", "0000 Staging.log")
    else null
  @inline private def xlog(x: String) =
    if (VERBOSE_SCHEDULING) withLog(graphLog) { log(x) }

  implicit class EdgeIdToInt(x: EdgeId) {
    @inline def toInt: Int = x.asInstanceOf[Int]
  }
  implicit class NodeIdToInt(x: NodeId) {
    @inline def toInt: Int = x.asInstanceOf[Int]
  }
  implicit class IntToEdgeId(x: Int) {
    @inline def toEdgeId: EdgeId = x.asInstanceOf[EdgeId]
  }
  implicit class IntToNodeId(x: Int) {
    @inline def toNodeId: NodeId = x.asInstanceOf[NodeId]
  }
  ...
  def checkIfAcyclic(root: Any, components: List[List[NodeId]]) =
  ...
  def nodeOutputs(node: NodeId): Seq[EdgeId] = {
    (NodeData.outputs(node.toInt).toInt until NodeData
      .outputs(node.toInt + 1)
      .toInt).map(_.toEdgeId)
  }
  ...
  def nodeTunnels(node: NodeId): Seq[(EdgeId, Seq[EdgeId])] =
    NodeData.tunnels(node.toInt)
  ...
  def triple(id: NodeId): Triple =
    (nodeOutputs(id).map(edgeOf), nodeOf(id), nodeInputs(id).map(edgeOf))
  ...
  final def addBound(out: Edge) =
    addNode(Nil, Seq(out), Nil, Nil, Nil, null).head

  /** Add output edge(s) with corresponding node **/
  final def addNode(ins: Seq[Edge],
                    outs: Seq[Edge],
                    binds: Seq[Edge],
                    tunnel: Seq[(Edge, Seq[Edge])],
                    freqs: Seq[UseFreq],
                    node: Node) = {
    outs.foreach { out =>
    ...
    ins.foreach { in =>
      ...
      if (!deps.contains(curNodeId)) {
        ...
        xlog(
          c"  Adding dependency to input $in = ${nodeOf(producerOf(in.id))}")
    ...
    NodeData.tunnels += tunnel.map {
      case (tun, results) => (tun.id.toEdgeId, results.map(_.id.toEdgeId))
    }
  ...
  private val comparator = new java.util.Comparator[(NodeId, Int)] {
    def compare(a: (NodeId, Int), b: (NodeId, Int)) =
      if (b._2 < a._2) -1 else if (b._2 == a._2) 0 else 1
  }
  ...
  def buildScopeIndex(scope: Iterable[NodeId]): OrderCache = {
    ...
    var idx   = 0
    for (node <- scope) {
      nodeOutputs(node).foreach { edge =>
        cache.put(edge.toInt, (node, idx))
      }
      idx += 1
    }
    cache
  ...
  def scheduleDepsWithIndex(roots: Iterable[EdgeId],
                            cache: OrderCache): Seq[NodeId] = {
    val sortedSet = new java.util.TreeSet[(NodeId, Int)](comparator)

    for (edge <- roots) {
      cache.get(edge.toInt).foreach { node =>
        sortedSet.add(node)
      }
    }
  ...
  def dfs(start: Iterable[NodeId])(
      succ: NodeId => Iterable[NodeId]): List[NodeId] = {
    ...
    def traverse(node: NodeId, tab: Int = 0) {
      if (!visit.contains(node)) {
        ...
        xlog(
          c"[DFS]" + "  " * tab + s"${triple(node)} -> " + successors
            .flatMap(nodeOutputs)
            .mkString(", "))
  ...
  /**
    ...
    */
  def sccs(start: Iterable[NodeId])(
      succ: NodeId => Iterable[NodeId]): List[List[NodeId]] = {
    var id: Int = 0
    val stack   = Stack()
    val mark    = SMap()
    ...
    def visit(node: NodeId): Int = {
      if (mark.contains(node.toInt)) mark(node.toInt)
        ...
        stack.addFirst(node)
        val min = succ(node).foldLeft(id) { (m, node) =>
          Math.min(m, visit(node))
        }
  ...
  def noCold(node: NodeId, scope: Set[NodeId]): Iterable[NodeId] = {
    nodeInputs(node)
      .zip(nodeFreqs(node))
      .filter(_._2 != Freq.Cold)
      .map(x => producerOf(x._1)) filter scope.contains
  }
  def noHot(node: NodeId, scope: Set[NodeId]): Iterable[NodeId] = {
    nodeInputs(node)
      .zip(nodeFreqs(node))
      .filter(_._2 != Freq.Hot)
      .map(x => producerOf(x._1)) filter scope.contains
  }

  // --- Visit methods (unscoped)
  def reverse(node: NodeId): Iterable[NodeId] =
    nodeInputs(node).map(producerOf)
  def forward(node: NodeId): Iterable[NodeId] =
    nodeOutputs(node).flatMap(dependentsOf)

  def getSchedule(roots: Iterable[NodeId],
                  cache: OrderCache,
                  checkAcyclic: Boolean = true): List[NodeId] = {
    if (checkAcyclic) {
      val xx = sccs(roots) { node =>
        ordered(node, cache)
      }
  ...
  def getBoundDependents(scope: Set[NodeId]): Set[NodeId] = {
    def getDependents(root: NodeId) = {
      ...
      def uses(node: NodeId) = {
        if (VERBOSE_SCHEDULING) {
          forward(node, scope).foreach { next =>
            xlog(
              c"[USES] ${triple(next)} (bounds = " + nodeBounds(next).mkString(
                ",") + ")")
          }
        }

        forward(node, scope) filterNot { next =>
    ...
    val boundDependents = scope.flatMap { node =>
      ...
      if (VERBOSE_SCHEDULING) {
        if (bounds.nonEmpty) {
          ...
          bounds.foreach { bnd =>
            xlog(c"  [Bounds]  ${triple(bnd)}")
          }
          xlog(c"  [Bounds] binded: ")
          sched.foreach { s =>
            xlog(c"  [Bounds]  ${triple(s)}")
          }
        }
      }
    ...
    val tunnelDependents = scope.flatMap { node =>
      if (VERBOSE_SCHEDULING) {
        if (nodeTunnels(node).nonEmpty)
          xlog(c"  [Tunnel] node: ${triple(node)}: ")
      }

      nodeTunnels(node).flatMap {
        case (tun, res) =>
  ...
  def getLocalScope(currentScope: Seq[NodeId],
                    result: Seq[EdgeId],
                    localCache: OrderCache): Seq[NodeId] = {
    ...
    def shouldOutside(x: NodeId) =
      reachableWarm.contains(x) || hotFringeDeps.contains(x)
...
// These xlog statements are VERY expensive - use only when debugging
    if (VERBOSE_SCHEDULING) {
      ...
      scope.foreach { stm =>
        xlog(c"  ${triple(stm)}");
        xlog(c"    dependents = ${nodeOutputs(stm).flatMap(dependentsOf)}")
      }
      xlog("For result: ")
      result.foreach { s =>
        xlog(c"  ${triple(producerOf(s))}")
      }
      xlog("Must inside: ")
      mustInside.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
      xlog("May outside: ")
      mayOutside.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
      xlog("Fringe: ")
      fringe.foreach { stm =>
      ...
      xlog("Reachable [Warm]: ")
      reachableWarm.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
      xlog("Reachable [Cold]: ")
      reachableCold.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
      xlog(s"Hot paths: ")
      hotPathsOnly.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
      ...
      xlog(s"Hot fringe: ")
      hotFringe.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
      xlog("Hot fringe dependencies: ")
      hotFringeDeps.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
      xlog("levelScope: ")
  ...
  def getLocalSchedule[A](availableNodes: Seq[NodeId],
                          result: Seq[EdgeId]): Seq[NodeId] = {
    ...
    val localSchedule = getSchedule(
      localRoots,
      localCache,
      checkAcyclic = false) filter (localScope contains _)

// These xlog statements are VERY expensive - use only when debugging
    if (VERBOSE_SCHEDULING) {
      xlog("avail nodes: ")
      availableNodes.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
      xlog("avail roots:")
      availRoots.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
      xlog("local nodes:")
      localNodes.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
      xlog("local scope: ")
      localScope.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
      xlog("local roots:")
      localRoots.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
      xlog("local schedule:")
      localSchedule.foreach { stm =>
        xlog(c"  ${triple(stm)}")
      }
    }
