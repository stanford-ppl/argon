...
object Boolean {
  ...
  @internal def const(x: CBoolean): Exp[MBoolean] = constant[MBoolean](x)

  /** Constructors **/
  @internal def not(x: Exp[Boolean]): Exp[Boolean] = x match {
    case Const(c)   => const(!c)          // Constant propagation
  ...
  @internal def and(x: Exp[MBoolean], y: Exp[MBoolean]): Exp[MBoolean] = (x,y) match {
    case (Const(a), Const(b)) => const(a && b)    // Constant propagation
