...
object IfThenElseOps {
  /** Constructors **/
  @internal def ifThenElse[T:Type](cond: Exp[Bool], thenp: () => Exp[T], elsep: () => Exp[T]): Exp[T] = cond match {
...
trait IfThenElseExp {
  ...
  @internal def __ifThenElse[A, B, T<:MetaAny[T]](cond: Bool, thenp: => A, elsep: => B)(implicit liftA: Lift[A,T], liftB: Lift[B,T]): T = {
  ...
  @internal def __ifThenElse[A, T<:MetaAny[T]](cond: Bool, thenp: => A, elsep: => T)(implicit lift: Cast[A, T], x1: Overload1): T = {
  ...
  @internal def __ifThenElse[A, T<:MetaAny[T]](cond: Bool, thenp: => T, elsep: => A)(implicit lift: Cast[A,T], x2: Overload2): T = {
    implicit val staged: Type[T] = lift.staged
  ...
  @internal def __ifThenElse[T<:MetaAny[T]:Type](cond: Bool, thenp: => T, elsep: => T): T = {
    val unwrapThen = () => thenp.s // directly calling unwrap(thenp) forces thenp to be evaluated here
    val unwrapElse = () => elsep.s // wrapping it as a Function0 allows it to be delayed
    wrap(ifThenElse(cond.s, unwrapThen, unwrapElse))
  }
