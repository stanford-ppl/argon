...
trait IndexPatternExp extends Staging {
  ...
  case class AffineAccess(a: Exp[Index], i: Bound[Index], b: Exp[Index])
      extends IndexPattern { def index = Some(i) }
  // i + b, where b must be loop invariant
  case class OffsetAccess(i: Bound[Index], b: Exp[Index])
      extends IndexPattern { def index = Some(i) }
  // a*i, where a must be loop invariant
  case class StridedAccess(a: Exp[Index], i: Bound[Index])
      extends IndexPattern { def index = Some(i) }
  ...
  case class AccessPattern(indices: Seq[IndexPattern])
      extends Metadata[AccessPattern] {
    def mirror(f: Tx) =
      AccessPattern(indices.map {
        case AffineAccess(a, i, b) =>
          AffineAccess(f(a), f(i).asInstanceOf[Bound[Index]], f(b))
        case OffsetAccess(i, b) =>
          OffsetAccess(f(i).asInstanceOf[Bound[Index]], f(b))
        case StridedAccess(a, i) =>
          StridedAccess(f(a), f(i).asInstanceOf[Bound[Index]])
  ...
  object accessPatternOf {
    def apply(x: Exp[_]): Seq[IndexPattern] =
      accessPatternOf.get(x).getOrElse {
        throw new UndefinedAccessPatternException(x)
      }
    def update(x: Exp[_], indices: Seq[IndexPattern]) {
      metadata.add(x, AccessPattern(indices))
    }
    def get(x: Exp[_]): Option[Seq[IndexPattern]] =
      metadata[AccessPattern](x).map(_.indices)
  }
}
...
trait AccessPatternAnalyzer extends Traversal {
  ...
  private def inLoop[T](loop: Exp[_], indices: Seq[Bound[Index]])(
      blk: => T): T = {
    val inner = innerStms
    indices.foreach { i =>
      innerScopes += i -> inner
  ...
  def indexUnapply(x: Exp[Index]): Option[Bound[Index]] = x match {
    case s: Bound[_] if loopIndices.contains(s.asInstanceOf[Bound[Index]]) =>
      Some(s.asInstanceOf[Bound[Index]])
    case _ => None
  }
  ...
  def isInvariant(b: Exp[Index], i: Bound[Index]): Boolean = b match {
    ...
    case Param(_) => true
    case s: Sym[_] =>
      !innerScopes(i).exists { stm =>
        stm.lhs.contains(s)
      }
    case _ => false
  }
  def isInvariantForAll(b: Exp[Index]): Boolean = loopIndices.forall { i =>
    isInvariant(b, i)
  }

  def extractIndexPattern(x: Exp[Index]) = x match {
    case Plus(Times(LoopIndex(i), a), b)
        if isInvariant(a, i) && isInvariant(b, i) =>
      AffineAccess(a, i, b) // i*a + b
    case Plus(Times(a, LoopIndex(i)), b)
        if isInvariant(a, i) && isInvariant(b, i) =>
      AffineAccess(a, i, b) // a*i + b
    case Plus(b, Times(LoopIndex(i), a))
        if isInvariant(a, i) && isInvariant(b, i) =>
      AffineAccess(a, i, b) // b + i*a
    case Plus(b, Times(a, LoopIndex(i)))
        if isInvariant(a, i) && isInvariant(b, i) =>
      AffineAccess(a, i, b) // b + a*i
    case Plus(LoopIndex(i), b) if isInvariant(b, i) =>
      OffsetAccess(i, b) // i + b
    case Plus(b, LoopIndex(i)) if isInvariant(b, i) =>
      OffsetAccess(i, b) // b + i
    case Times(LoopIndex(i), a) if isInvariant(a, i) =>
      StridedAccess(a, i) // i*a
    case Times(a, LoopIndex(i)) if isInvariant(a, i) =>
      StridedAccess(a, i) // a*i
