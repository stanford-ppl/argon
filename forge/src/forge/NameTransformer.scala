...
class NameTransformer[Ctx <: blackbox.Context](val c: Ctx) {
  ...
  private class Tx(var subst: Map[String,Any]) extends Transformer {
    ...
    def apply(x: String): String = {
      ...
      val result = subst.foldRight(x){case ((orig,repl),cur) => cur.replaceAll(orig,repl.toString) }
      //c.info(c.enclosingPosition, s"Created $result", true)
      result
    }

    def apply(x: TypeName): TypeName = x match {case TypeName(x) => TypeName(f(x)) }
    def apply(x: TermName): TermName = x match {case TermName(x) => TermName(f(x)) }
    def apply(x: Name): Name = x match {
      case x: TypeName => f(x)
      case x: TermName => f(x)
    }
    def apply[T<:Tree](x: T): T = transform(x).asInstanceOf[T]
    def apply[T<:Tree](xs: List[T]): List[T] = xs.map{x => f(x) }
    def fs[T<:Tree](xss: List[List[T]]): List[List[T]] = xss.map{xs => f(xs) }

    override def transform(tree: Tree): Tree = atPos(tree.pos) { tree match {
      case ClassDef(mods,name,parents,template)     => ClassDef(mods,f(name),f(parents),f(template))
      case TypeDef(mods,name,tpars,rhs)             => TypeDef(mods,f(name),f(tpars),f(rhs))
      case DefDef(mods,name,tpars,vparamss,tp,body) => DefDef(mods,f(name),f(tpars),fs(vparamss),f(tp),f(body))
