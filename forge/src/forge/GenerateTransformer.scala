...
class GenerateTransformer[Ctx <: blackbox.Context](val c: Ctx) {
  ...
  def apply[T <: Tree](x: T): List[T] = {
    ...
    while (level.exists(needsUnroll) && round < 20) {
      val nextLevel = level.flatMap { x =>
        if (needsUnroll(x)) transformer.t(x) else List(x)
      }
      level = nextLevel
      round += 1
    }
    if (level.exists(needsUnroll)) {
      c.warning(
        c.enclosingPosition,
        "Generation unrolling did not appear to complete after 20 iterations.")
    }
    level
  }
  ...
  object Ranged {
    ...
    def unapply(x: String): Option[(String, String, Range)] = x match {
      case Numbered(name, pattern, num) =>
        Some((name, pattern, 0 until num.toInt))
      case Until(name, pattern, start, end) =>
        Some((name, pattern, start.toInt until end.toInt))
      case To(name, pattern, start, end) =>
        Some((name, pattern, start.toInt to end.toInt))
      case _ => None
    }
    ...
    def unapply(x: TypeName): Option[(TypeName, String, Range)] = x match {
      case TypeName(Ranged(name, pattern, range)) =>
        Some((TypeName(name), pattern, range))
      case _ => None
    }
    def unapply(x: TermName): Option[(TermName, String, Range)] = x match {
      case TermName(Ranged(name, pattern, range)) =>
        Some((TermName(name), pattern, range))
      case _ => None
    }
  }
  object Repeat {
    ...
    def unapply(x: String): Option[(String, Range)] = x match {
      ...
      case Until(pattern, start, end) =>
        Some((pattern, start.toInt until end.toInt))
  ...
  private class Tx extends Transformer {
    ...
    implicit class RangeTx(x: Range) {
      def tx[T](p: String)(func: Int => T): List[T] =
        x.map { i =>
    ...
    private def checkName(orig: String, repl: String): Unit = {
      if (orig forall Character.isDigit) {
        c.error(
          c.enclosingPosition,
          s"Cannot unroll ${orig}, as this would create an illegal name $repl")
      }
    }

    def f(x: String): String = subst.foldRight(x) {
      case ((orig, repl), cur) => cur.replaceAll(orig, repl.toString)
    }
    def f(x: TypeName): TypeName = x match {
      case TypeName(x) =>
        val name = f(x)
        checkName(x, name)
        TypeName(name)
    }
    def f(x: TermName): TermName = x match {
      case TermName(x) =>
        val name = f(x)
        checkName(x, name)
        TermName(name)
    }
    def f(x: Name): Name = x match {
      case x: TypeName => f(x); case x: TermName => f(x)
    }
    def f[T <: Tree](x: T): T = transform(x).asInstanceOf[T]
    def f[T <: Tree](xs: List[T]): List[T] = xs.map { x =>
      f(x)
    }
    ...
    def t[T <: Tree](xs: List[T]): List[T] = xs.flatMap { x =>
      t(x)
    }
    def ts[T <: Tree](xss: List[List[T]]): List[List[T]] = xss.map { xs =>
      t(xs)
    }
    ...
    def unroll[T <: Tree](tree: T): List[T] = tree match {
      case ValDef(mods, Ranged(name, p, range), tp, rhs) =>
        range.tx(p) { i =>
          ValDef(mods, f(name), f(tp), f(rhs)).asInstanceOf[T]
        }

      case ModuleDef(mods, Ranged(name, p, range), template) =>
        range.tx(p) { i =>
          ModuleDef(mods, f(name), f(template)).asInstanceOf[T]
        }
      ...
      case DefDef(mods,
        ...
        range.tx(p) { i =>
          DefDef(mods,
                 ...
                 ts(vparamss),
                 f(returnType),
                 f(body)).asInstanceOf[T]
        }

      case TypeDef(mods, Ranged(name, p, range), tparams, rhs) =>
        range.tx(p) { i =>
          TypeDef(mods, f(name), t(tparams), f(rhs)).asInstanceOf[T]
        }

      // case X$X$0to4 =>
      case CaseDef(Ident(Ranged(name, p, range)), guard, body) =>
        range.tx(p) { i =>
          CaseDef(f(Ident(name)), f(guard), f(body)).asInstanceOf[T]
        }

      // case xX$X$0to4 =>
      case CaseDef(Bind(Ranged(name, p, range), Typed(expr, tpt)),
                   guard,
                   body) =>
        range.tx(p) { i =>
          CaseDef(Bind(f(name), Typed(f(expr), f(tpt))), f(guard), f(body))
            .asInstanceOf[T]
        }

      // case x: TypeX$X$0to4 =>
      case CaseDef(Bind(x, Typed(expr, Ident(Ranged(name, p, range)))),
                   guard,
                   body) =>
        range.tx(p) { i =>
          CaseDef(Bind(f(x), Typed(f(expr), f(Ident(name)))),
                  f(guard),
                  f(body)).asInstanceOf[T]
        }
      ...
      case CaseDef(
          Bind(
            x,
            Typed(expr, AppliedTypeTree(Ident(Ranged(name, p, range)), args))),
          guard,
          body) =>
        range.tx(p) { i =>
          CaseDef(
            Bind(f(x),
                 Typed(f(expr), AppliedTypeTree(f(Ident(name)), t(args)))),
            f(guard),
            f(body)).asInstanceOf[T]
        }
      ...
      case Bind(Ranged(name, p, range), body) =>
        range.tx(p) { i =>
          Bind(f(name), f(body)).asInstanceOf[T]
        }

      case Select(Ident(Ranged(name, p, range)), select) =>
        range.tx(p) { i =>
          Select(f(Ident(name)), f(select)).asInstanceOf[T]
        }

      case Select(qualifier, Ranged(name, p, range)) =>
        range.tx(p) { i =>
          Select(f(qualifier), f(name)).asInstanceOf[T]
        }

      case Ident(Ranged(name, p, range)) =>
        range.tx(p) { i =>
          f(Ident(name)).asInstanceOf[T]
        }
    ...
    override def transform(tree: Tree): Tree = atPos(tree.pos) {
      tree match {
        case ClassDef(mods, name, tpars, template) =>
          ClassDef(mods, f(name), t(tpars), f(template))
        case TypeDef(mods, name, tpars, rhs) =>
          TypeDef(mods, f(name), t(tpars), f(rhs))
        case DefDef(mods, name, tpars, vparamss, tp, body) =>
          DefDef(mods, f(name), t(tpars), ts(vparamss), f(tp), f(body))
        case ValDef(mods, name, tp, rhs) =>
          ValDef(mods, f(name), f(tp), f(rhs))
        case Template(parents, selfType, body) =>
          Template(t(parents), f(selfType), t(body))
