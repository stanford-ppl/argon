...
trait Traversal extends CompilerPass with BlockTraversal { self =>
  ...
  final protected def runSingle[S:BStaged](b: Block[S]): Block[S] = {
    val b2 = preprocess(b)
  ...
  /**
    ...
    */
  protected def process[S:BStaged](block: Block[S]): Block[S] = runSingle(block)
  protected def preprocess[S:BStaged](block: Block[S]): Block[S] = { block }
  protected def postprocess[S:BStaged](block: Block[S]): Block[S] = { block }
...
trait IterativeTraversal extends Traversal {
  ...
  final protected def runIterative[S:BStaged](b: Block[S]): Block[S] = {
  ...
  override protected def process[S:BStaged](b: Block[S]): Block[S] = runIterative(b)
}
