...
trait IfThenElseExp extends Staging with BoolExp with VoidExp {
  ...
  /** IR Nodes **/
  case class IfThenElse[T:FStaged](cond: Exp[Bool], thenp: Block[T], elsep: Block[T]) extends Op[T] {
  ...
  /** Constructors **/
  def ifThenElse[T:FStaged](cond: Exp[Bool], thenp: => Exp[T], elsep: => Exp[T])(implicit ctx: SrcCtx): Exp[T] = cond match {
