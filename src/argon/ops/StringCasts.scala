...
trait StringCastExp extends StringCastOps with TextExp with MixedNumericExp with ArgonExceptions {
  ...
  /** IR Nodes **/
  case class StringToFixPt[S:BOOL,I:INT,F:INT](x: Sym[Text]) extends FixPtOp[S,I,F] {
    def mirror(f:Tx) = text_to_fixpt[S,I,F](x)
  }
  case class StringToFltPt[G:INT,E:INT](x: Sym[Text]) extends FltPtOp[G,E] {
    def mirror(f:Tx) = text_to_fltpt[G,E](x)
  }
  case class StringToBool(x: Sym[Text]) extends Op[Bool] {
    def mirror(f:Tx) = text_to_bool(x)
  }

  /** Smart Constructors **/
  def text_to_fixpt[S:BOOL,I:INT,F:INT](x: Sym[Text])(implicit ctx: SrcCtx): Sym[FixPt[S,I,F]] = x match {
    case Const(c: String) => string2fixpt[S,I,F](c).s
    case _ => stage(StringToFixPt[S,I,F](x))(ctx)
  }
  def text_to_fltpt[G:INT,E:INT](x: Sym[Text])(implicit ctx: SrcCtx): Sym[FltPt[G,E]] = x match {
    case Const(c: String) => string2fltpt[G,E](c).s
    case _ => stage(StringToFltPt[G,E](x))(ctx)
  }
  def text_to_bool(x: Sym[Text])(implicit ctx: SrcCtx): Sym[Bool] = x match {
