...
trait ArrayApi extends ArrayExp with FixPtApi with VoidApi with TextApi {
  ...
  def Array[T:FStaged](size: Int32)(implicit ctx: SrcCtx): ArgonArray[T] = ArgonArray(array_new[T](size.s))
}

trait ArrayExp extends Staging with FixPtExp with VoidExp with TextExp {
  /** Infix methods **/
  case class ArgonArray[T:FStaged](s: Exp[ArgonArray[T]]) {
  ...
  /** Type classes **/
  // --- FStaged
  case class ArrayType[T](child: FStaged[T]) extends FStaged[ArgonArray[T]] {
  ...
  implicit def arrayType[T:FStaged]: FStaged[ArgonArray[T]] = ArrayType(ftyp[T])

  /** IR Nodes **/
  case class InputArguments() extends Op[ArgonArray[Text]] { def mirror(f:Tx) = stage(InputArguments())(here) }

  case class ArrayNew[T:FStaged](size: Exp[Int32]) extends Op2[T,ArgonArray[T]] {
    def mirror(f:Tx) = array_new[T](f(size))
  }

  case class ArrayApply[T:FStaged](array: Exp[ArgonArray[T]], i: Exp[Int32]) extends Op[T] {
    def mirror(f:Tx) = array_apply(f(array),f(i))
    override def aliases = Nil
    //override def extracts = syms(array) TODO: Why does this cause issues?
  }

  case class ArrayLength[T:FStaged](array: Exp[ArgonArray[T]]) extends Op[Int32] {
    def mirror(f:Tx) = array_length(f(array))
  }

  /** Constructors **/
  def array_new[T:FStaged](size: Exp[Int32])(implicit ctx: SrcCtx): Sym[ArgonArray[T]] = {
    stageMutable(ArrayNew[T](size))(ctx)
  }
  def array_apply[T:FStaged](array: Exp[ArgonArray[T]], i: Exp[Int32])(implicit ctx: SrcCtx): Sym[T] = {
    stage(ArrayApply(array,i))(ctx)
  }

  def array_length[T:FStaged](array: Exp[ArgonArray[T]])(implicit ctx: SrcCtx): Sym[Int32] = {
