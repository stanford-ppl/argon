...
trait BoolExp extends BoolOps with BitsExp {
  ...
  /** IR Nodes **/
  case class Not(a: Sym[Bool]) extends Op[Bool] { def mirror(f:Tx) = bool_not(f(a)) }
  case class And(a: Sym[Bool], b: Sym[Bool]) extends Op[Bool]  { def mirror(f:Tx) = bool_and(f(a), f(b)) }
  case class Or(a: Sym[Bool], b: Sym[Bool]) extends Op[Bool]   { def mirror(f:Tx) = bool_or(f(a), f(b)) }
  case class XOr(a: Sym[Bool], b: Sym[Bool]) extends Op[Bool]  { def mirror(f:Tx) = bool_xor(f(a), f(b)) }
  case class XNor(a: Sym[Bool], b: Sym[Bool]) extends Op[Bool] { def mirror(f:Tx) = bool_xnor(f(a), f(b)) }
  case class RandomBool() extends Op[Bool] { def mirror(f:Tx) = bool_random() }


  /** Smart Constructors **/
  def bool_not(x: Sym[Bool])(implicit ctx: SrcCtx): Sym[Bool] = x match {
  ...
  def bool_and(x: Sym[Bool], y: Sym[Bool])(implicit ctx: SrcCtx): Sym[Bool] = (x,y) match {
  ...
  def bool_or(x: Sym[Bool], y: Sym[Bool])(implicit ctx: SrcCtx): Sym[Bool] = (x,y) match {
    case (Const(a:Boolean),Const(b:Boolean)) => bool(a || b)       // Constant propagation
  ...
  def bool_xor(x: Sym[Bool], y: Sym[Bool])(implicit ctx: SrcCtx): Sym[Bool] = (x,y) match {
    case (Const(a:Boolean),Const(b:Boolean)) => bool(a != b)           // Constant propagation
    case (Const(false), b)                   => b                      // Boolean simplification: false != b => b
  ...
  def bool_xnor(x: Sym[Bool], y: Sym[Bool])(implicit ctx: SrcCtx): Sym[Bool] = (x,y) match {
    case (Const(a:Boolean),Const(b:Boolean)) => bool(a == b)           // Constant propagation
