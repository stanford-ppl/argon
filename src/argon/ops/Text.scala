...
trait TextExp extends Staging with BoolExp {
  ...
  case class Text(s: Exp[Text]) {
    ...
    def +[T:FStaged](that: T)(implicit ctx: SrcCtx): Text = Text(text_concat(this.s, textify(that).s))
  ...
  /** Direct methods **/
  def textify[T:FStaged](x: T)(implicit ctx: SrcCtx): Text = Text(sym_tostring(x.s))


  /** Virtualized methods **/
  def infix_toString[S:FStaged](x: S)(implicit ctx: SrcCtx): Text = textify(x)
  def infix_+[R:FStaged](x1: String, x2: R)(implicit ctx: SrcCtx): Text = string2text(x1) + textify(x2)
  def infix_+[R:FStaged](x1: R, x2: String)(implicit ctx: SrcCtx): Text = textify(x1) + string2text(x2)
  ...
  /** IR Nodes **/
  case class ToString[S:FStaged](x: Exp[S]) extends Op[Text] { def mirror(f:Tx) = sym_tostring(f(x)) }
  ...
  /** Constructors **/
  def sym_tostring[S:FStaged](x: Exp[S])(implicit ctx: SrcCtx): Exp[Text] = x match {
