...
trait CastExp extends Staging with NumExp {
  ...
  implicit class CastOps[T:FStaged:Num](x: T) {
    def to[R:FStaged:Num](implicit ctx: SrcCtx): R = cast[T,R](x)
  }

  implicit class ConstCastOps[T:Numeric](x: T) {
    def as[R:FStaged:Num](implicit ctx: SrcCtx): R = castLift[R](x)
  }

  protected def cast[T:FStaged:Num,R:FStaged:Num](x: T)(implicit ctx: SrcCtx): R = (num[T],num[R]) match {
  ...
  protected def castLift[R:FStaged:Num](x: Any)(implicit ctx: SrcCtx): R = {
