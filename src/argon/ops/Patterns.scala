...
trait PatternExp extends Staging with FixPtExp with VoidExp with TextExp /*{
  ...
  abstract class ParallelCollection[A <: StageAny[A] : Staged]
  ...
  /** Foreach elem **/
  case class ForeachElem[A <: StageAny[A] : Staged](
  ...
  abstract class CollectOutput[A <: StageAny[A] : Staged, I:Staged, CA <: StageAny[A] : Staged] {

  }

  abstract class CollectBaseElem[A <: StageAny[A] : Staged, R <: StageAny[R] : Staged](
  ...
  case class CollectElem[A <: StageAny[A] : Staged, I<:ParallelCollection[A]:Staged, CA<:ParallelCollection[A]:Staged](
  ...
  /** Constructors **/
  def foreach_elem[A <: StageAny[A] : Staged](func: => Exp[A], chunks: Int)(implicit ctx: SrcCtx): Sym[Void] = {
    val blk = stageBlock{ func }
    val effects = blk.summary
    stageEffectful(ForeachElem(blk, chunks), effects.star)(ctx)
  }

  def collect_elem[A <: StageAny[A] : Staged, I<:ParallelCollection[A]:Staged, CA<:ParallelCollection[A]:Staged](
    buffer:      CollectOutput[A,I,CA],
