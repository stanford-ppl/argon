...
trait StructExp extends Staging with VoidExp {
  ...
  case class FieldApply[S <: StageAny[S] :StructType,T: Staged](struct: Exp[S], field: String) extends Op2[S,T] {
    def mirror(f:Tx) = field_apply[S,T](f(struct), field)

    override def extracts = syms(struct)
  }
  case class FieldUpdate[S <: StageAny[S] : StructType,T: Staged](struct: Exp[S], field: String, value: Exp[T]) extends Op3[S,T,Void] {
  ...
  def field_apply[S <: StageAny[S] :StructType,T: Staged](struct: Exp[S], index: String)(implicit ctx: SrcCtx): Exp[T] = struct match {
  ...
  def field_update[S <: StageAny[S] : StructType,T: Staged](struct: Exp[S], index: String, value: Exp[T])(implicit ctx: SrcCtx): Exp[Void] = {
  ...
  def unwrapStruct[S <: StageAny[S]:StructType,T: Staged](struct: Exp[S], index: String): Option[Exp[T]] = struct match {
    case Op(Struct(elems)) => elems.get(index) match {
      case Some(x) if x.tp <:< ftyp[T] => Some(x.asInstanceOf[Exp[T]]) // TODO: Should this be Staged asInstanceOf?
      case None =>
