...
trait StructExp extends Staging with VoidExp {
  ...
  abstract class StructApi[T:StructType] { self =>
    def field[R:FStaged](name: String)(implicit ctx: SrcCtx): R = wrap(field_apply[T,R](unwrap(self.asInstanceOf[T]), name))
  }

  abstract class StructType[T] extends FStaged[T] {
  ...
  def field[T:StructType,R:FStaged](struct: T, name: String)(implicit ctx: SrcCtx): R = wrap(field_apply[T,R](struct.s, name))
  ...
  case class FieldApply[S:StructType,T:FStaged](struct: Exp[S], field: String) extends Op2[S,T] {
    def mirror(f:Tx) = field_apply[S,T](f(struct), field)

    override def extracts = syms(struct)
  }
  case class FieldUpdate[S:StructType,T:FStaged](struct: Exp[S], field: String, value: Exp[T]) extends Op3[S,T,Void] {
  ...
  def field_apply[S:StructType,T:FStaged](struct: Exp[S], index: String)(implicit ctx: SrcCtx): Exp[T] = struct match {
  ...
  def field_update[S:StructType,T:FStaged](struct: Exp[S], index: String, value: Exp[T])(implicit ctx: SrcCtx): Exp[Void] = {
  ...
  def unwrapStruct[S:StructType,T:FStaged](struct: Exp[S], index: String): Option[Exp[T]] = struct match {
