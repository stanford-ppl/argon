...
trait TupleExp extends Staging with StructExp with NumExp with BitsExp {
  ...
  /** Infix methods **/
  case class Tup2[A:FStaged,B:FStaged](s: Exp[Tup2[A,B]]) extends StructApi[Tup2[A,B]] {
    def _1(implicit ctx: SrcCtx): A = field[A]("_1")
    def _2(implicit ctx: SrcCtx): B = field[B]("_2")
  }

  /** Direct methods **/
  def pack[A:FStaged,B:FStaged](a: A, b: B)(implicit ctx: SrcCtx): Tup2[A,B] = struct[Tup2[A,B]]("_1" -> a.s, "_2" -> b.s)
  def pack[A:FStaged,B:FStaged](t: (A, B))(implicit ctx: SrcCtx): Tup2[A,B] = struct[Tup2[A,B]]("_1" -> t._1.s, "_2" -> t._2.s)
  def unpack[A:FStaged,B:FStaged](t: Tup2[A,B])(implicit ctx: SrcCtx): (A,B) = (t._1, t._2)

  /** Virtualized methods **/
  def infix_toString[A:FStaged,B:FStaged](x: Tup2[A,B])(implicit ctx: SrcCtx): Text = "Tup2(" + textify(x._1) + ", " + textify(x._2) + ")"

  /** Type classes **/
  // --- FStaged
  case class Tup2Type[A,B](m1: FStaged[A], m2: FStaged[B]) extends StructType[Tup2[A,B]] {
  ...
  implicit def tup2Type[A:FStaged,B:FStaged]: StructType[Tup2[A,B]] = Tup2Type(ftyp[A],ftyp[B])

  // --- Bits
  class Tup2Bits[A:FStaged:Bits,B:FStaged:Bits] extends Bits[Tup2[A,B]] {
  ...
  implicit def __tup2Bits[A:FStaged:Bits,B:FStaged:Bits]: Bits[Tup2[A,B]] = new Tup2Bits[A,B]
  ...
  // --- Num
  class Tup2Arith[A:FStaged:Arith,B:FStaged:Arith] extends Arith[Tup2[A,B]] {
  ...
  implicit def __tup2Arith[A:FStaged:Arith,B:FStaged:Arith]: Arith[Tup2[A,B]] = new Tup2Arith[A,B]
}
