...
trait TupleExp extends Staging with StructExp with NumExp with BitsExp {
  ...
  /** Type classes **/
  // --- Staged
  case class Tup2Type[A,B](m1: Staged[A], m2: Staged[B]) extends StructType[Tup2[A,B]] {
  ...
  implicit def tup2Type[A:Staged,B:Staged]: StructType[Tup2[A,B]] = Tup2Type(typ[A],typ[B])

  // --- Bits
  class Tup2Bits[A:Staged:Bits,B:Staged:Bits] extends Bits[Tup2[A,B]] {
  ...
  implicit def __tup2Bits[A:Staged:Bits,B:Staged:Bits]: Bits[Tup2[A,B]] = new Tup2Bits[A,B]
  ...
  // --- Num
  class Tup2Arith[A:Staged:Arith,B:Staged:Arith] extends Arith[Tup2[A,B]] {
  ...
  implicit def __tup2Arith[A:Staged:Arith,B:Staged:Arith]: Arith[Tup2[A,B]] = new Tup2Arith[A,B]
}
